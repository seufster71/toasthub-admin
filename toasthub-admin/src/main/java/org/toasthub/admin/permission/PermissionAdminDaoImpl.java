...
public class PermissionAdminDaoImpl extends PermissionDaoImpl implements PermissionAdminDao {
	...
	public void delete(RestRequest request, RestResponse response) throws Exception {
		if (request.containsParam(GlobalConstant.ITEMID) && !"".equals(request.getParam(GlobalConstant.ITEMID))) {
			...
			Permission permission = (Permission) entityManagerSecuritySvc.getInstance().getReference(Permission.class,  request.getParamLong(GlobalConstant.ITEMID));
	...
	public void rolePermissionSave(RestRequest request, RestResponse response) throws Exception {
		...
		if (rolePermission.getRole() == null) {
			Role role = (Role) entityManagerSecuritySvc.getInstance().getReference(Role.class,  request.getParamLong(GlobalConstant.PARENTID));
			rolePermission.setRole(role);
		}
		if (rolePermission.getPermission() == null) {
			Permission permission = (Permission) entityManagerSecuritySvc.getInstance().getReference(Permission.class,  request.getParamLong("permissionId"));
